...
namespace AndersonPay.Controllers
{
    public class AndersonPayController : Controller
    {
        ...
        public ActionResult Index()
            // nasa loob ng parenthesis ng Index - string sortBy, int? page, string filterBy, int? id, string status, string searchBy, string search
        {
            //Search Function
            //if (!String.IsNullOrEmpty(search))
            //{
                return View();
               // db.invoices.Where(x => x.Name.StartsWith(search) || search == null).ToList().ToPagedList(page ?? 1, 30)
            //}
